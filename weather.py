...
root =Tk() # It creates the main window for the application
root.geometry("400x400") #set the size of the window
root.resizable(0,0) # Disable the window's resizing feature
root.title("Weather App") # title for the window

def time_format_for_location(utc):
    local_time = datetime.utcfromtimestamp(utc) # unix timestamp to datetime object
...
def showWeather():
    ...
    response = requests.get(weather_url) # sends a request to the api and gets the response 
    weather_info = response.json() # changing response from json to python readable
    tfield.delete('1.0', 'end')   # to clear the text field for every new output

#as per API documentation, if the status code('cod') is 200, it means that weather data was successfully fetched
